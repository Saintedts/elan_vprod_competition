...
def preprocess(df, is_train: bool = True, train_df: pd.DataFrame = None):
    ...
    if is_train:
        ...
        encoded_required_drive_license = explode_and_encode_train(df, 'required_drive_license',
                                                                  'required_drive_license')
        encoded_languageKnowledge = explode_and_encode_train(df, 'languageKnowledge', 'languageKnowledge',
                                                             'code_language')
        df = pd.concat([df.drop(columns=['required_drive_license', 'languageKnowledge']),
                        encoded_required_drive_license,
                        encoded_languageKnowledge], axis=1)
        ...
        encoded_required_drive_license = explode_and_encode(df, 'required_drive_license', 'required_drive_license')
        encoded_languageKnowledge = explode_and_encode(df, 'languageKnowledge', 'languageKnowledge', 'code_language')
        df = pd.concat([df.drop(columns=['required_drive_license', 'languageKnowledge']),
                        encoded_required_drive_license,
                        encoded_languageKnowledge], axis=1)
